...
export function calculateBoundaryElementNM(input = {}, ctx = {}) {
  ...
  const cb =
    (epsCU / (epsCU + fy / Math.max(Es, 1))) * d;
  ...
  const curveNominal =
    appendZeroPoint(sortByN(nominalRaw));

  const curveDesign =
    appendZeroPoint(sortByN(designRaw));
  ...
  const lintelReaction =
    positive(ctx.lintelReaction);

  const lintelEcc =
    Math.abs(finite(ctx.lintelEcc));

  const psiE =
    positive(ctx.psiE, 0.30);
  ...
  const r =
    Ag_m2 > 0 ? Math.min(Ab_m2 / Ag_m2, 1) : 0;

  const Gb =
    r *
    (finite(ctx.Gwall) + finite(ctx.GlineTotal));

  const Qb =
    r * finite(ctx.QlineTotal);
...
export function calculateInPlaneDesign(input = {}) {
  ...
  const gamma =
    positive(input.concreteDensity, 24);
  ...
  const Sp = finite(
      input.structuralPerformanceFactor,
      1.3 - 0.3 * mu
    );
  ...
  const Vseismic =
    ...
    Wt *
    positive(
      input.seismicDistributionFactor,
      1
    );
  ...
  if (aspectRatio < 1) {
    wallClassification = 'Squat wall';
  } else if (aspectRatio > 2) {
    wallClassification = 'Slender wall';
  }
  ...
  const bearingArea =
    (positive(input.bearingWidth) / 1000) *
    (positive(input.bearingLength) / 1000);

  const bearingStress = bearingArea > 0
      ? kNToN(lintelReaction) /
        (bearingArea * 1e6)
      : 0;
  ...
  const nVerticalBars = VbarSpace > 0
      ? Math.floor(bmm / VbarSpace) + 1
      : 0;

  const AsDistributed =
    nVerticalBars * vBarArea;
  ...
  const AsHorizontalPerM =
    HbarSpace > 0
      ? hBarArea * 1000 / HbarSpace
      : 0;

  const AsBoundary =
    positive(input.boundaryBarCount) *
    areaBar(
      positive(input.boundaryBarDiameter)
    );

  const boundaryArea =
    positive(input.boundaryWidth) *
    1000 *
    positive(
  ...
  const rhoBoundary =
    boundaryArea > 0
      ? AsBoundary / boundaryArea
      : 0;

  const boundarySteelTensionCapacity =
    AsBoundary * fy / 1000;
  ...
  const boundaryNM =
    calculateBoundaryElementNM(input, {
      Gwall,
      GlineTotal,
      QlineTotal,
      Ngravity,
  ...
  const phiPn =
    boundaryNM.available
      ? boundaryNM.keyPoints.phiP0
      : 0;

  const phiMn =
    boundaryNM.available
      ? boundaryNM.keyPoints.phiM0
      : 0;

  const axialRatio = phiPn > 0
      ? NseismicCompression / phiPn
      : boundaryNM.available
        ? Infinity
        : 0;
   ...
   */
  const AvPerM = VbarSpace > 0
      ? 2 * vBarArea * 1000 / VbarSpace
      : 0;
  ...
  const shearRatio = shearCapacity > 0
      ? Vtotal / shearCapacity
      : Vtotal > 0
        ? Infinity
        : 0;
...
export function calculateOutOfPlaneDesign(input = {}) {
  ...
  const fyMesh =
    positive(input.fyMesh);

  const fc =
    positive(input.fc);

  const Es =
    positive(input.Es, 200000);

  const Vbar =
    positive(input.VbarDia);

  const Vspace =
    Math.max(
      positive(input.VbarSpace),
      0.001
    );

  const Hbar =
    positive(input.HbarDia);

  const Hspace =
    Math.max(
      positive(input.HbarSpace),
      0.001
    );

  const Fbar =
    positive(input.FootBarDia);

  const Fspace =
    Math.max(
      positive(input.FootBarSpace),
      0.001
    );

  const cover =
    positive(input.cover);

  const AsMesh =
    positive(input.MeshArea);

  const supportWS =
    input.supportWindSeismic ||
    input.outOfPlaneSupportCondition ||
    'Pinned-Pinned';
  ...
  const wsFactors =
    getSupportFactors(supportWS);

  const fireFactors =
    getSupportFactors(supportFire);
